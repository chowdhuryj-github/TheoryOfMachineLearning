...
# Problem 1
## Practice KNN by hand.
_Note: This problem needs to be done on paper, not in a notebook._

Given the follwoing feature matrix, compelte the next steps for the KNN algorithm.
...
![Graph paper](Images/KNN%20Part%20II.png)

2. Using KNN with K = 3, fill in the missing values in the table. Note: KNN can be used to predict the label fo a feature vector or to fill in the missing entries of a column in the feature matrix. In this question, use regression KNN to predict the missing entries of feature 3 for the following 3 data samples. To predict the class of each data sample, rely only on the first two features and use classification KNN (you would normally need to rely on the three features to predict the class label, but for brevity use only the first two features).
...
When a model does not perform well on the training set, we know that the model underfits the training set. When a model performs very well on the training set, but fails to shopw similar performance on the test set, we conclude that the model overfits the training set (hence the importance of splitting the data into training and test sets).
